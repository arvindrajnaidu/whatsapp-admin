...
function getDb() {
  ...
  stmts = {
    ...
    upsertKnownChat: db.prepare(`
      INSERT OR REPLACE INTO known_chats (jid, name, type, last_seen)
      VALUES (@jid, @name, @type, @lastSeen)
    `),
    listKnownChats: db.prepare(`SELECT * FROM known_chats ORDER BY last_seen DESC`),
...
export function upsertKnownChat(jid, name, type) {
  const { stmts } = getDb();
  stmts.upsertKnownChat.run({ jid, name, type, lastSeen: Date.now() });
}

export function listKnownChats() {
  const { stmts } = getDb();
  return stmts.listKnownChats.all();
}
